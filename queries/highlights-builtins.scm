...
(
  ...
  (
    ...
    (#match? @constant.clef "^(GG|alto(?:(?:varC)?)|ba(?:ritone(?:(?:var(?:[CF]))?)|ss)|french|hufnagel\-do\-fa|kievan\-do|m(?:e(?:nsural\-(?:[fg])|zzosoprano)|oderntab)|pe(?:rcussion|trucci\-(?:[fg]))|s(?:oprano|ubbass)|t(?:ab|enor(?:(?:G|varC)?)|reble)|v(?:ar(?:C|baritone|percussion)|iolin)|[CFG])$")
  )
)
...
(
  ...
  (
    ...
    (#match? @constant  "^\\\\(aeolian|dorian|ionian|l(?:(?:ocr|yd)ian)|m(?:ajor|i(?:nor|xolydian))|phrygian)$")
  )
)
...
(
  ...
  (
    (symbol) @constant
    (#match? @constant "^(percent|segno|tremolo|unfold|volta)$")
  )
)
...
(
  ...
  (expression_block
    (
      ...
      (#match? @constant "^\\\\(cm|in|mm|pt|staff\-space)$")
    )
  )
)
...
(
  ...
  (
    (symbol) @constant
    (#match? @constant "^(arabic|catal(?:an|à)|deutsch|e(?:nglish|spa(?:(?:[nñ])ol))|français|italiano|n(?:ederlands|orsk)|portugu(?:(?:[eê])s)|s(?:uomi|venska)|vlaams)$")
  )
)
...
(
  ...
  (expression_block
    (assignment_lhs
      (symbol) @variable
      (#match? @variable "^(arranger|co(?:mposer|pyright)|d(?:edication|octitle)|instrument|lsrtags|meter|opus|p(?:iece|oet)|sub(?:(?:(?:sub)?)title)|t(?:agline|exidoc|itle))$")
    )
  )
)
